...
FIXME [![Clojars Project](https://clojars.org/dev.gethop/user-manager.cognito/latest-version.svg)](https://clojars.org/dev.gethop/user-manager.cognito)
...
- Issuer URL:  It will have the following structure: `https://cognito-idp.{region}.amazonaws.com/{userPoolId}`
- Client id: The id of the app-client that the library will use to authentiate. It's important that the app-client to be used can't have a `client-secret`. Clients with a `secret` are intendend to be used at the backend side.
...
#### Logout
``` clojure
(rf/dispatch [::session.logout/user-logout
                   {:on-success-evt [::success]
				    :on-failure-evt [::failure]}])
```
#### Forgot password
